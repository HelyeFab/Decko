...
Planned, postponed until after MVP_010.

## Why this was renumbered

This brief was originally drafted as `MVP_009 — Study Options and Deck Overrides`.

That sequence changed after real Anki deck screenshots exposed a more fundamental import problem:

```text
MVP_009 must preserve the full imported Anki source.
MVP_010 must map preserved card templates into distinct Decko study cards.
Only then do study options make sense.
```

So this work is now MVP_011.
...
By the time this MVP starts, Decko should already have:
...
- lossless imported Anki source data
- note-type-aware card mapping for templates such as Listening, Reading, and Production
- deck detail and review session flows

MVP_011 should introduce a first-class options layer without changing FSRS maths.
...
MVP_011 should add deck-level control:
...
MVP_012 should expand from simple deck overrides toward a more Anki-like options/profile system only after MVP_011 is stable.